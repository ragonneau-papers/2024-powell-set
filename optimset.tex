...
\begin{abstract}
    This paper shows the optimality of an interpolation set proposed by Powell for derivative-free trust-region methods.
...
\Gls{dfo} methods, i.e., optimization methods that do not rely on derivative information, are required to solve the optimization problem
\begin{equation}
    \label{eq:nlp}
...
Powell proposed an interpolation set~$\xpt[0]$ that is employed by many interpolation-based \gls{dfo} methods, including \solvername{uobyqa}, \solvername{newuoa}, and if there are no constraints, \solvername{bobyqa}, \solvername{lincoa}, and \solvername{cobyqa}.
