...
Clearly, if the interpolation conditions~\eqref{eq:intp-cond} do not contradict each others,~$\objm[k]$ is uniquely defined by~\eqref{eq:intp-cond} only if
\begin{equation*}
    \card\xpt[k] = \dim\qpoly = \frac{1}{2} (n + 1) (n + 2).
\end{equation*}
This scheme, referred to as fully-determined interpolation, is the one employed by \solvername{uobyqa}.
...
This mechanism prevents the methods from performing unnecessary function evaluations, which are normally considered as the principal measure of complexity of \gls{dfo} methods.
...
It consists indeed of the origin and equidistant points to the origin on the positive and negative coordinate directions.
...
\end{equation*}
where~$\lagp[i]$ is the~$i$th minimum-norm Lagrange polynomial associated with~$\xpt[0]_m$ (see Definition~\ref{def:min-norm-lagp}) for~$i \in \set{1, 2, \dots, m}$.
