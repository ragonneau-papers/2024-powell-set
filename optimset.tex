...
To ensure that~$\objm[k]$ is uniquely defined by~\eqref{eq:intp-var} and includes second-order information, one normally requires that
\begin{equation*}
...
If~$\xpt$ is~$\Lambda_0$-poised in the minimum Frobenius norm sense in~$\mathcal{C}$, it is obviously~$\Lambda$-poised in the same sense for all~$\Lambda \ge \Lambda_0$.
