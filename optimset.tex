...
This mechanism tries to make the best of the function evaluations, which are normally considered as the principal cost of \gls{dfo} methods.
In such a case, it is clear that the initial interpolation set~$\xpt[0]$ must be carefully chosen, as it will impact the optimization method for the subsequent iterations.
...
\begin{proof}
    ...
    \begin{equation*}
        \Lambda_p = \max_{\|x\|_p\le \Delta}|\lagp[1](x)| \ge \max_{t \in \R} \set[\bigg]{1 - \frac{1}{\Delta} (2n + 1 - m) t : (2n + 1 - m)^{\frac{1}{p}}\abs{t} \le \Delta} = 1 + (2n + 1 - m)^{\frac{p - 1}{p}}.
    \end{equation*}
...
There are more cases where we can calculate the exact value of~$\Lambda_p$, as will be detailed in the following.
...
%\begin{proof}
%    According to Theorem~\ref{thm:bounds},~$\Lambda_1$ is lower bounded by the right-hand side of~\eqref{eq:lambda-p-1}.
...
It is interesting to ask whether~$2n + 1$ still minimizes~$\Lambda_p$ for any~$p \ge 1$ after the extension. %, which seems to be true according to some numerical experiments.
