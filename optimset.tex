...
\begin{definition}[Poisedness]
    The set~$\xpt$ is \emph{poised} in the minimum Frobenius norm sense if the solution to problem~\eqref{eq:intp-cond-gen} exists and is unique for any real-valued function~$\obj$.
\end{definition}
...
% \end{equation*}
Observe that the KKT system for the variational problem~\eqref{eq:intp-cond-gen} is a linear system,
because this is an equality-constrained quadratic programming problem.
...
\begin{theorem}
    \label{th:optimset}
    Assume that~$m=2n+1$, and either~$n\le 2$ or
    \[
...
The fact that~$\Lambda_p = 1$ for~$p \in \set{1, 2}$ is, in fact, shown also by Propositions~\ref{prop:lambda-p-1} and~\ref{prop:lambda-p-2}.
