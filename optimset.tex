...
% \newcommand{\reviewed}[1]{#1}

% Headings
\title{An Optimal Interpolation Set for Model-Based Derivative-Free Optimization Methods}
\author{
    Tom M. Ragonneau\thanks{
        Department of Applied Mathematics, The Hong Kong Polytechnic University, Hong Kong\reviewed{, China}.\\
...
\reviewed{It is also employed by~\cite{Conejo_Karas_Pedroso_2015} in a method for constrained problems.}
...
\begin{proof}
    ...
    \end{equation*}
    Since~$\lagp$ and~\reviewed{$\lagp[i]$} are quadratic polynomials sharing the same values on~$\set{y^1, @@ y^{j + 1}, @@ y^{n + j + 1}}$, \reviewed{Taylor expansions of the quadratic polynomial~$\lagp$ around~$y^1$ yield}
    \begin{equation*}
        \big(\nabla^2 \lagp \big)_{j, j} = \frac{\lagp(y^{j + 1}) + \lagp(y^{n + j + 1}) - 2 \lagp(y^1)}{\Delta^2} = \big(\nabla^2 \lagp[i] \big)_{j, j}.
    \end{equation*}
    On the other hand, it is easy to check \reviewed{according to~\eqref{eq:lagp}} that all the entries of~$\nabla^2 \lagp[i]$ are zero except for the first~$m-n-1$ diagonal entries.
    Therefore,
    \begin{equation*}
...
\begin{proof}
    We will establish the bounds \reviewed{in~\eqref{eq:bounds}} using the formulation of~$\Lambda_p$ in Lemma~\ref{lem:lambda-p}.
    ...
    Therefore, we only need to show that~$\Lambda_{\infty} \le \reviewed{\max @ \set{n - 1, @@ 2n - m + 2}} \le n$.
    ...
    Thus
    \begin{equation*}
        \Lambda_{\infty} = \max_{\norm{x}_{\infty} \le \Delta} @@ \abs[\big]{\lagp[1](x)} \le \reviewed{\max @ \set{n - 1, @@ 2n - m + 2}},
    \end{equation*}
    which completes the proof \reviewed{of~\eqref{eq:bounds}.
...
\reviewed{Theorem~\ref{thm:bounds} provides the value of~$\Lambda_{\infty}$}, which is either~$n - 1$ or~$n$ since~$m \ge n + 2$.
There are more cases where we can \reviewed{evaluate}~$\Lambda_p$, as we will detail in the following.
...
\reviewed{Theorem~\ref{thm:optimset} shows that the equality holds for a larger range of~$p$, because the right-hand side in~\eqref{eq:pineq} is greater than~$2$ when~$n > 2$.}
...
We have analyzed the well-poisedness \reviewed{(in the minimum Frobenius norm sense)} of an interpolation set that appears in many trust-region \gls{dfo} methods, particularly those by Powell\reviewed{~\cite{Powell_2006,Powell_2009}}.
...
\reviewed{Zaikun Zhang would like to thank the late Professor Oleg Burdakov for his friendship.}

\paragraph*{Disclosure statement}
The authors report that there are no competing interests to declare.

\paragraph*{Funding} This work was funded by the University Grants Committee of Hong Kong under the projects PF18-24698 (Hong Kong PhD Fellowship Scheme), PolyU 253012/17P, PolyU 153054/20P, and PolyU 153066/21P. It was also supported by The Hong Kong Polytechnic University under project P0009767.
