...
Theorem~\ref{thm:lambda-p}\fxnote{This is not true anymore} is the main result of this paper and provides explicit bounds for the constant of poisedness of the interpolation set under study.
...
We develop bounds on its constant of poisedness, and evaluate this constant in some special cases.
Finally, we point out that the set is is optimal in terms of the poisedness in a ball centered at the starting point.
...
%We will show that~$\xpt[0]_{2n + 1}$ is optimal in the sense that it achieves the least possible
%constant of poisedness in~$\mathcal{B}_p(\Delta)$ for all~$p \in [1, 2]$.
