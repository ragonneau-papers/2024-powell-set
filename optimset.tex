...
% Reviewing
\newcommand{\reviewed}[1]{\textcolor{NavyBlue}{#1}}
...
\end{equation*}
Such a scheme, referred to as the fully-determined quadratic interpolation, is used by UOBYQA~\cite{Powell_2002}.
...
\begin{proof}
    ...
    \end{equation*}
    Since~$\lagp$ and~$\lagp[i]$ are quadratic polynomials sharing the same values on~$\set{y^1, y^{j + 1}, y^{n + j + 1}}$, \reviewed{the Taylor expansion of the quadratic polynomial~$\lagp$ around~$y^1$ yields}
    \begin{equation*}
...
\begin{proof}
    ...
    \end{equation*}
    \reviewed{Since both these bounds can be reached in~$\mathcal{B}_{\infty}(\Delta)$, we thus have}
    \begin{equation*}
        \Lambda_{\infty} = \max_{\norm{x}_{\infty} \le \Delta} \abs[\big]{\lagp[1](x)} \reviewed{= \max \set{n - 1, 2n - m + 2}} \le n,
    \end{equation*}
    which completes the proof.
\end{proof}
...
\paragraph*{Funding} This work was funded by the University Grants Committee of Hong Kong under the projects PF18-24698 (Hong Kong Ph.D. Fellowship Scheme), PolyU 253012/17P, PolyU 153054/20P, and PolyU 153066/21P. It was also supported by The Hong Kong Polytechnic University under project P0009767.
